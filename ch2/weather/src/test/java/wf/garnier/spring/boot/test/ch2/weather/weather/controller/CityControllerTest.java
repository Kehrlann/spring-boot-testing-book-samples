...
class CityControllerTest {
	...
	void shouldAddCityToPreferred() throws Exception {
		...
		// When/Then
		mockMvc.perform(post("/cities/1/prefer")).andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/"));
	...
	void shouldReturn404WhenAddingNonExistentCity() throws Exception {
		...
		// When/Then
		mockMvc.perform(post("/cities/999/prefer")).andExpect(status().isNotFound());
	}
